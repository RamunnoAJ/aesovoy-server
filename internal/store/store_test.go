...
func setupTestDB(t *testing.T) *sql.DB {
	...
	_, err = db.Exec(`TRUNCATE order_products, orders, product_ingredients, products, categories, providers, provider_categories, clients, tokens, users, ingredients, payment_methods, local_stock, local_sales, local_sale_items RESTART IDENTITY CASCADE`)
	require.NoError(t, err)
	return db
}
